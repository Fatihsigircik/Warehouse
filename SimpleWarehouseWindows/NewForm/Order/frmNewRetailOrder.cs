...
{
    public partial class frmNewRetailOrder : BaseForm
    {
        ...
        Dictionary<string, List<ProductVariant>> variantDictionary = new Dictionary<string, List<ProductVariant>>();
         ...
         List<OrderDetailHelper> _detail = new List<OrderDetailHelper>();

        private Dictionary<int, RefAction<OrderDetailHelper>> functionDictionary;

        private int _id;
        private static Dictionary<int, frmNewRetailOrder> _newOrder = new Dictionary<int, frmNewRetailOrder>();
        private List<Currency> _currencies;
        private bool _isLoading = true;


        public static frmNewRetailOrder GetInstance(int id = 0)
        {
            if (!_newOrder.ContainsKey(id))
                _newOrder.Add(id, new frmNewRetailOrder(id));
            return _newOrder[id];
        }

        public static void RemoveInstance(frmNewRetailOrder frm)
        {
        ...
        public frmNewRetailOrder(int id)
        {
            _id = id;
            IsOnlyOne = true;
            InitializeComponent();
        ...
        private async Task LoadOrder()
        {
            ...
            ktxtCompanyName.Text = order.CompanyName;
            ktxtCustomerName.Text = order.CustomerName;
            ktxtCustomerSurname.Text = order.CustomerSurname;
        ...
        private async Task SetOrderDetail(OrderDetail orderDetail)
        {
            ...
            _detail.Add(new OrderDetailHelper(/*currencies, warehouses, units*/)
            {
        ...
        {
            kdgvProductDetail.DataSource = new BindingSource() { DataSource = _detail };
        ...
        private void FrmNewOrderWarehouse_SelectedIndexChanged(object sender, EventArgs e)
        {
            if ((sender as ComboBox).SelectedItem == null)
                return;
            _detail[kdgvProductDetail.CurrentRow.Index].Warehouse = (sender as ComboBox).SelectedItem as Warehouse;
        ...
        private void FrmNewOrder_SelectedIndexChanged(object sender, EventArgs e)
        {
            if ((sender as ComboBox).SelectedItem == null)
                return;
            if (kdgvProductDetail.CurrentCell.ColumnIndex == 6)
            {
                _detail[kdgvProductDetail.CurrentRow.Index].Currency = (sender as ComboBox).SelectedItem as Currency;
                (kdgvProductDetail.CurrentRow.DataBoundItem as OrderDetailHelper).Currency =
                    (sender as ComboBox).SelectedItem as Currency;
            }
            //else if (kdgvProductDetail.CurrentCell.ColumnIndex == 8)
            //{
        ...
        private Helpers.Models.FullOrder GetOrder()
        {
            ...
            {
                CurrencyId = _detail[0].Currency.CurrencyId,
                CreatedDate = DateTime.Now,
                CreatedId = SettingsHelper.UserName.UserGroupId,
                IsDeleted = false,
                CustomerId = -1,
                CompanyName = ktxtCompanyName.Text,
                CustomerName = ktxtCustomerName.Text,
                CustomerSurname = ktxtCustomerSurname.Text,
                ...
                OrderTypeId = 1,
                TotalPrice = _detail.Sum(q => q.Price * q.Quantity),
                TotalVal = _detail.Sum(q => (q.Price - (q.Price / (1M + ((decimal)q.VatPercent / 100M)))) * q.Quantity),
                OrderDetails = GetDetails()
            };
            return fullOrder;
        }
        private List<OrderDetail> GetDetails()
        {
            ...
            return _detail.Select(q => q as OrderDetail).ToList();
        }

        private async void frmNewRetailOrder_Load(object sender, EventArgs e)
        {
            if (_id > 0)
            {
                OpenLoader();
        ...
        private void Pd_PrintPage(object sender, PrintPageEventArgs e)
        {
            ...
            if (curPage == 1)
            {
                ...
                e.Graphics.DrawString($"Belge Numarası : {ktxtDocNumber.Text}", myFont, Brushes.Black,
                    leftMargin, yPos, new StringFormat());
                e.Graphics.DrawString($"Firma Tel      : {ktxtPhone.Text}", myFont, Brushes.Black,
                    leftMargin + 400, yPos, new StringFormat());
                i++;
                yPos = topMargin + i * myFont.GetHeight(e.Graphics);
            ...
            while ((i < nLines) && (counter < _detail.Count))
            {
                // Get line for output from richTextBox1
                curLine = _detail[counter];
