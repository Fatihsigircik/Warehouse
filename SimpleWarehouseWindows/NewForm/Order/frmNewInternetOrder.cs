...
{
    public partial class frmNewInternetOrder : BaseForm
    {
        ...
        private int _id;
        private static Dictionary<int, frmNewInternetOrder> _newOrder = new Dictionary<int, frmNewInternetOrder>();
        private List<Currency> _currencies;
        private bool _isLoading = true;
        public static frmNewInternetOrder GetInstance(int id = 0)
        {
            if (!_newOrder.ContainsKey(id))
                _newOrder.Add(id, new frmNewInternetOrder(id));
            return _newOrder[id];
        }

        public static void RemoveInstance(frmNewInternetOrder frm)
        {
        ...
        public frmNewInternetOrder(int id)
        {
            _id = id;
            IsOnlyOne = true;
            InitializeComponent();
        ...
        {
            functionDictionary = new Dictionary<int, frmNewInternetOrder.RefAction<OrderDetailHelper>>()
            {
                {0,SelectProduct}
            };
        }
        ...
        private bool ValidateOrder(ref string msg)
        {
            ...
            if (ktxtInvoiceAddress.Text.Length == 0)
            {
                msgList.Add("Lütfen Fatura Adresini Giriniz.");
                retVal = false;
            }
        ...
        private void Pd_PrintPage(object sender, PrintPageEventArgs e)
        {
            ...
            if (curPage == 1)
            {
                ...
                e.Graphics.DrawString($"Belge Numarası : {ktxtDocNumber.Text}", myFont, Brushes.Black,
                    leftMargin, yPos, new StringFormat());
                e.Graphics.DrawString($"Müşteri Adı    : {ktxtCustomerName.Text} {ktxtCustomerSurname.Text}", myFont, Brushes.Black,
                    leftMargin + 400, yPos, new StringFormat());
                i++;
                yPos = topMargin + i * myFont.GetHeight(e.Graphics);
